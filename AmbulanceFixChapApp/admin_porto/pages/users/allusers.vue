                ...
                <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                    <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                        ...
                        <div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                            <button  class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800" style="background-color:teal">
                                <svg class="h-3.5 w-3.5 mr-2" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                    <path clip-rule="evenodd" fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" />
                                </svg>
                                <NuxtLink to="/users/create">Add new user</NuxtLink> 
                            </button>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3">Nida</th>
                            ...
                            <tbody>
                                <tr v-for="user in users" :key="user.id" class="border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-3">{{ user.nida }}</td>
                                    <td class="px-4 py-3">{{ user.route }}</td>
                                    <td class="px-4 py-3">{{ user.role }}</td>
                                    <td class="flex items-center justify-end px-4 py-3">
                                      <!-- Action buttons -->
                                      <NuxtLink :to="`/users/${user.id}`" class="px-3 py-1 text-white bg-green-500 rounded-md hover:bg-green-600">Edit</NuxtLink>
                                      <button type="button" @click="deleteUser(user.id)" class="px-3 py-1 ml-2 text-white bg-red-500 rounded-md hover:bg-red-600">Delete</button>
                                    </td>
                                  </tr>
...
// Define a reactive variable to store users
const users = ref([]);
...
async function fetchUsers() {
  try {
    const response = await axios.get('http://127.0.0.1:8000/api/usersv7');
...
async function deleteUser(userId) {
  try {
    ...
    // Optionally, you can remove the deleted user from the users list
