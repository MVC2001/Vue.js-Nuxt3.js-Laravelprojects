...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['per_id']) && !empty($_POST['per_id'])) {
        ...
        $deleteQuery = mysqli_query($connect, "DELETE FROM `permission` WHERE `per_id` = $per_id");
        
        if ($deleteQuery) {
            // User deleted successfully
            header('Location: permission.php'); // Redirect to users page
            exit();
        } else {
            // Error deleting user
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <button class="btn btn-dark btn-block" style="width:150px;background-color:#032B58"><a href="addPermission.php">AddPermission</a></button>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example2" class="display table-bordered" style="width:100%">
                                        ...
                                        <tbody>
                                         ...
                                         <?php 
                 $count=1;
                 $select_all = "SELECT * FROM `permission` ORDER BY per_id DESC";
                 ...
                 if ($number > 0) {
                     while($row = mysqli_fetch_assoc($result)) { ?>
                            ...
                            <td><?php echo $count ?></td>
                                ...
                                <td>
                                    <a href="updatePermission.php?per_id=<?php echo $row['per_id'] ?>">
