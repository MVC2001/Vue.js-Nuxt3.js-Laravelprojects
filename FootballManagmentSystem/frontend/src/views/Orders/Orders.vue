<template>
  <div class="flex">
    ...
    <div class="w-1/2 md:w-1/4 lg:w-1/5 min-h-screen text-white bg-gray-800 text-white p-4 fixed" style="background-color:teal;box-shadow: rgba(0, 0, 0, 0.19) 0px 10px 20px, rgba(0, 0, 0, 0.23) 0px 6px 6px;">
      <ul class="p-4">
        <li><a href="#">ALL CARTS</a></li><hr><br>
      ...
      <div v-else>
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
          <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400" style="box-shadow: rgba(9, 30, 66, 0.25) 0px 1px 1px, rgba(9, 30, 66, 0.13) 0px 0px 1px 1px;">
            ...
            <tbody>
              <tr v-for="cartv1 in carts" :key="cartv1.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ cartv1.item }}</td>
                <td class="px-6 py-4">{{ cartv1.category }}</td>
                <td class="px-6 py-4">{{ cartv1.price }}</td>
                <td class="px-6 py-4">{{ cartv1.accountno }}</td>
                <td class="px-6 py-4">{{ cartv1.paymethod }}</td>
                <td class="px-6 py-4">{{ cartv1.amount }}</td>
                <td class="px-6 py-4">{{ cartv1.total }}</td>
                <td class="px-6 py-4">{{ cartv1.created_at }}</td>
                <td class="px-6 py-4 text-right">
                  <span class="flex">
                    <button @click="deleteCart(cartv1.id)" class="bg-red-500 text-white px-4 py-2 mt-2">Delete</button>
                    <router-link :to="'/carts/' + cartv1.id + '/edit-orderById'">
...
export default {
  ...
  try {
    await axios.delete(`http://127.0.0.1:8000/api/addToCartApi/${cartId}`);
    // Instead of fetching all carts after deletion, you can directly remove the deleted cart from the local carts array
    this.carts = this.carts.filter(cartv1 => cartv1.id !== cartId);
  } catch (error) {
