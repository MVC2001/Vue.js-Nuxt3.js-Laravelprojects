    ...
    <div class="flex h-screen bg-gray-100">
      ...
      <aside class="hidden w-64 text-gray-100 bg-gray-800 lg:block">
        ...
        <nav class="py-5 p-7" style="margin-top: 90px;">
          <ul class="py-4">
            ...
            <li>
              <nuxt-link to="/facilities" class="block px-6 py-2 hover:bg-gray-700">Manage-facilities</nuxt-link>
            </li>
          </ul>
        </nav>
      ...
      <aside class="fixed top-0 left-0 z-50 w-64 h-screen text-gray-100 bg-gray-800" :class="{ 'block': isSidebarOpen, 'hidden': !isSidebarOpen }">
        ...
        <nav class="py-5 p-7" style="margin-top: 60px;">
          <ul class="py-4">
            <li>
              <nuxt-link to="/dashboard" class="block px-6 py-2 hover:bg-gray-700">AdminDashboard</nuxt-link>
            </li>
            <li>
              <nuxt-link to="/reset_password" class="block px-6 py-2 hover:bg-gray-700">Reset-Password</nuxt-link>
            </li>
      ...
      <main class="relative flex-1">
        ...
        <div class="absolute inset-0">
          <img src="@/assets/images/webimg.png" alt="Background Image" class="object-cover w-full h-full" />
          <div class="absolute inset-0 bg-black opacity-50"></div> <!-- Optional overlay for better text readability -->
        </div>

        <!-- Top navigation -->
        <header class="relative z-10 flex items-center justify-between p-4 bg-white border-b border-gray-200">
          <div>
          ...
          <!-- Logout button -->
          <button @click="logout" class="px-4 py-2 text-white bg-gray-800 rounded hover:bg-red-600">Logout</button>
        </header>
        <!-- End of top navigation -->

        <!-- Page content -->
        <div class="relative z-10 text-center shadow-lg py-550 to-p-4 text-success" style="margin-top: 50px; color: teal; font-size: 35px;">
          <!-- Check if user is logged in and authenticated -->
          <h4>NEARBY HEALTH FACILITY LOCATOR</h4>
        </div>
        <!-- End of page content -->
      </main>
      <!-- End of main content -->
