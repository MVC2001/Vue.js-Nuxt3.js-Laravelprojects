<template>
    <div class="flex h-screen">
      ...
      <div class="flex flex-col flex-1 lg:ml-64">
        <header class="flex items-center justify-between p-4 text-white bg-teal-800" style="box-shadow: rgba(0, 0, 0, 0.16) 0px 3px 6px, rgba(0, 0, 0, 0.23) 0px 3px 6px;">
          <button @click="toggleSidebar" class="text-white lg:hidden">☰</button>
          <h1 class="text-2xl">DOWNLOAD SLIP HERE</h1>
          <div class="relative">
            <button @click="toggleDropdown" class="p-2 bg-gray-700 rounded hover:bg-gray-600">{{ userEmail }} ▼</button>
        ...
        <main class="p-4">
          <div>
            <a href="">
              <div class="px-4 py-2 font-bold text-white bg-teal-800 border border-teal-700 rounded card hover:bg-teal-700" style="width: 100%;">Your result slip</div>
            </a>
          </div>
              ...
              <tbody>
                <tr v-for="sleep in filteredSleeps" :key="sleep.id">
                  ...
                  <td class="px-4 py-2 border-b">
                    <a :href="`/storage/${sleep.file}`" target="_blank" class="text-blue-500 hover:text-blue-700">View File</a>
                  </td>
                  <td class="px-4 py-2 border-b">
                    <button class="px-4 py-2 font-bold text-white bg-teal-800 border border-teal-700 rounded hover:bg-teal-700">{{ sleep.status }}</button>
                  </td>
                  <td class="px-4 py-2 border-b">
                    <button @click="downloadSleep(sleep.file)" class="text-blue-500 hover:text-blue-700">
                      Download
                    </button>
                  </td>
                </tr>
  ...
  const fetchSleeps = async () => {
    try {
      ...
      filteredSleeps.value = response.data;
    } catch (error) {
  ...
  const filterSleeps = () => {
    ...
    filteredSleeps.value = sleeps.value.filter(sleep =>
      sleep.name.toLowerCase().includes(query) ||
      sleep.index_number.toLowerCase().includes(query) ||
      sleep.year.toLowerCase().includes(query)
    );
  };
